...
namespace Chrome_Dev_x86_Launcher
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            if (File.Exists(applicationPath + "\\Chrome Dev x86\\Chrome.exe"))
            {
                ...
                if (!File.Exists(applicationPath + "\\Chrome Dev x86\\Profile.txt"))
                {
                    Application.EnableVisualStyles();
                    Application.SetCompatibleTextRenderingDefault(false);
                    Application.Run(new Form1());
                    String Arguments = File.ReadAllText(applicationPath + "\\Chrome Dev x86\\Profile.txt") + sb.ToString();
                    if (Arguments.Contains("--user-data-dir="))
                    {
                        ...
                        Process.Start(applicationPath + "\\Chrome Dev x86\\Chrome.exe", Arguments3);
                    }
                    else
                    {
                        Process.Start(applicationPath + "\\Chrome Dev x86\\Chrome.exe", Arguments);
                    }
                }
                else
                {
