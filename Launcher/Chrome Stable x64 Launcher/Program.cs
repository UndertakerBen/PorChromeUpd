...
namespace Chrome_Stable_x64_Launcher
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            if (File.Exists(applicationPath + "\\Chrome Stable x64\\Chrome.exe"))
            {
                ...
                if (!File.Exists(applicationPath + "\\Chrome Stable x64\\Profile.txt"))
                {
                    Application.EnableVisualStyles();
                    Application.SetCompatibleTextRenderingDefault(false);
                    Application.Run(new Form1());
                    String Arguments = File.ReadAllText(applicationPath + "\\Chrome Stable x64\\Profile.txt") + sb.ToString();
                    if (Arguments.Contains("--user-data-dir="))
                    {
                        ...
                        Process.Start(applicationPath + "\\Chrome Stable x64\\Chrome.exe", Arguments3);
                    }
                    else
                    {
                        Process.Start(applicationPath + "\\Chrome Stable x64\\Chrome.exe", Arguments);
                    }
                }
                else
                {
