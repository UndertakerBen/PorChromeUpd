    ...
    static class Program
    {
        ...
        static void Main()
        {
            ...
            if (File.Exists(applicationPath + "\\Chrome\\Chrome.exe"))
            {
                ...
                if (!File.Exists(applicationPath + "\\Chrome\\Profile.txt"))
                {
                    Application.EnableVisualStyles();
                    Application.SetCompatibleTextRenderingDefault(false);
                    Application.Run(new Form1());
                    String Arguments = File.ReadAllText(applicationPath + "\\Chrome\\Profile.txt") + sb.ToString();
                    if (Arguments.Contains("--user-data-dir="))
                    {
                        ...
                        Process.Start(applicationPath + "\\Chrome\\Chrome.exe", Arguments3);
                    }
                    else
                    {
                        Process.Start(applicationPath + "\\Chrome\\Chrome.exe", Arguments);
                    }
                }
                else
                {
